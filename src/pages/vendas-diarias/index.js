...
const VendasDiaria = () => {
  ...
  const filtrarVendasPorData = () => {
    if (!dataInicio || !dataFim) {
      CustomToast({
        type: "error",
        mensage: "Por favor, selecione ambas as datas (início e fim)",
      });
      return vendas;
    }
    ...
    if (new Date(dataInicio) > new Date(dataFim)) {
      CustomToast({
        type: "error",
        mensage: "A data de início não pode ser maior que a data final",
      });
      return vendas;
    }
  ...
  const salvarEdicao = async () => {
    try {
      const formaPagamento =
        vendaEditando.forma_pagamento !== undefined
  ...
  const calcularTotais = () => {
    ...
    const vendasParaCalculo =
      dataInicio && dataFim ? filtrarVendasPorData() : vendas;
  ...
  const dadosTabela = (
    ...
    valor: venda.valor
      ? `R$ ${parseFloat(venda.valor).toFixed(2).replace(".", ",")}`
      : "R$ 0,00",
    formaPagamento:
      mapearFormaPagamento(venda.formaPagamento || venda.forma_pagamento) || "",
    total:
      venda.quantidade && venda.valor
        ? `R$ ${(venda.quantidade * venda.valor).toFixed(2).replace(".", ",")}`
        : "R$ 0,00",
  ...
  const handleDelete = async (row) => {
    try {
      ...
      setVendas((prevVendas) =>
        prevVendas.filter((venda) => venda.id !== row.id)
      );
        ...
        >
          ...
          <div className="flex gap-2 flex-wrap items-center w-full mt-6 justify-center md:justify-start p-2 ">
            ...
            <TextField
              ...
              autoComplete="off"
              sx={{ width: { xs: "25%", sm: "50%", md: "40%", lg: "9%" } }}
            />
            ...
            <TextField
              ...
              autoComplete="off"
              sx={{ width: { xs: "40%", sm: "50%", md: "40%", lg: "13%" } }}
            />
            ...
            <Autocomplete
              ...
              value={
                categoriasFiltradas.find(
                  (cat) => cat.id.toString() === categoriaSelecionada
                ) || null
              }
            ...
            <div className="w-full flex items-end justify-center md:justify-end ">
              <div className="w-[100%] flex items-center justify-center flex-wrap gap-2 md:justify-start">
                <label className="font-xs w-full font-bold mt-2 md:mt-0 text-center md:text-start">
                  Filtro
                </label>
                <TextField
                  fullWidth
            ...
            <div className="flex w-full items-center justify-center gap-2 flex-wrap">
              {/* Card Pix */}
              <div
                className="w-[45%] md:w-[17%] justify-center gap-8 flex items-center"
                style={{
              ...
              {/* Card Total */}
              <div
                className="w-[60%] md:w-[17%] justify-center gap-8 flex items-center mr-5"
                style={{
                  border: "1px solid #0D2E43",
                  borderRadius: "10px",
                  padding: "10px",
        ...
        conteudo={
          <div className="flex gap-2 flex-wrap items-end justify-end w-full mt-2">
            ...
            <SelectTextFields
              ...
              fontWeight={500}
              value={
                vendaEditando?.forma_pagamento?.toString() ||
                vendaEditando?.formaPagamento?.toString() ||
                ""
              }
              onChange={(e) =>
                setVendaEditando({
            ...
            <TextField
              ...
              value={
                vendaEditando?.data_venda
                  ? new Date(vendaEditando.data_venda)
                      .toISOString()
                      .split("T")[0]
                  : data
              }
