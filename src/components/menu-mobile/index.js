...
const MenuMobile = () => {
    ...
    try {
      ...
      if (Array.isArray(response)) {
        const unidadesFormatadas = response
          .filter((u) =>
            userData?.unidades?.some((userUnidade) => userUnidade.id === u.id)
          )
          .map((u) => ({
            value: u.id,
            label: u.nome,
        ...
        if (unidadesFormatadas.length > 0 && !unidadeId) {
          atualizarUnidade(
            unidadesFormatadas[0].value,
            unidadesFormatadas[0].ativo
          );
        }
      }
    } catch (error) {
  ...
  return (
    <div
      className="w-[100%] flex items-center justify-center p-5 gap-10 z-30 md:hidden mt-10"
      style={{ backgroundColor: "#d2d7db" }}
    >
      <div className="w-[42%] md:w-[25%] sm:mr-8 lg:w-[25%] md:mr-6 justify-center flex p-2 bg-white rounded-md">
        <SelectTextFields
          width={"150px"}
      ...
      >
        ...
        >
          <DashboardIcon style={{ color: "#0d2d43" }} />
          Dashboard
        </MenuItem>
        {tipoUsuario !== "3" && (
          <>
            ...
            >
              <AddchartIcon style={{ color: "#0d2d43" }} />
              Vendas
            </MenuItem>
            <MenuItem
            ...
            >
              <AddBusinessIcon style={{ color: "#0d2d43" }} />
              Vendas Diárias
            </MenuItem>
          </>
        )}
        {tipoUsuario !== "3" && (
          ...
          >
            <AddToQueueIcon style={{ color: "#0d2d43" }} />
            Saídas
          </MenuItem>
        )}
        <MenuItem
        ...
        >
          <DataThresholdingIcon style={{ color: "#0d2d43" }} />
          Relatório
        </MenuItem>
        ...
        >
          <MiscellaneousServicesIcon style={{ color: "#0d2d43" }} />
          Cadastro
        </MenuItem>
        <MenuItem
